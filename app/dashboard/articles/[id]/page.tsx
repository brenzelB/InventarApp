      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-6">
          ...
          <div className="flex items-start gap-8">
            ...
            <div>
              <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100 tracking-tight">{article.name}</h1>
              <p className="text-slate-500 dark:text-slate-400 font-medium">
                {article.sku} 
                {article.lagerort && (
                  <span className="ml-2 px-2 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-2xl text-xs font-bold text-accent uppercase tracking-wider">
                    {article.lagerort}
                  </span>
                )}
                <span className="mx-2">•</span>
                {isLowStock ? <span className="text-red-500 dark:text-rose-400 font-bold">Niedriger Bestand!</span> : <span className="text-green-600 dark:text-emerald-400 font-bold">Bestand OK</span>}
              </p>
            </div>
          </div>
        </div>
        
        <div className="flex flex-wrap gap-2 p-1 bg-slate-100 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700">
          {[
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm relative overflow-hidden group">
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    <div className="relative p-6 bg-slate-50 dark:bg-slate-950 rounded-[2.5rem] ring-1 ring-inset ring-slate-100/50 dark:ring-slate-800/50">
              ...
              <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm">
                ...
                <button 
                  ...
                  className="mt-6 w-full py-3 text-sm font-bold text-accent bg-indigo-50 dark:bg-indigo-900/30 rounded-3xl hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors"
                >
            ...
            <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
              {comments.length === 0 && (
                <div className="p-8 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 rounded-2xl flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-amber-800 dark:text-amber-300">Kommentar-Funktion möglicherweise inaktiv</p>
          ...
          <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm space-y-6">
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-600 dark:text-slate-400 font-semibold">Herstellpreis</span>
                <span className="font-bold text-slate-900 dark:text-slate-100">{Number(article.herstellpreis).toFixed(2)} €</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-600 dark:text-slate-400 font-semibold">Einkaufspreis</span>
                <span className="font-bold text-slate-900 dark:text-slate-100">{Number(article.purchase_price || 0).toFixed(2)} €</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-600 dark:text-slate-400 font-semibold">Verkaufspreis</span>
                <span className="font-bold text-slate-900 dark:text-slate-100">{Number(article.verkaufspreis).toFixed(2)} €</span>
              </div>
              <div className="flex justify-between items-center text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest pt-1">
