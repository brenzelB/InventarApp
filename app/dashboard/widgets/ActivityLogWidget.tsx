  ...
  return (
    <div className="h-full w-full bg-white dark:bg-slate-900 rounded-3xl p-8 shadow ring-1 ring-slate-200 dark:ring-slate-800 flex flex-col">
      ...
      <div className="flex-1 min-h-0 overflow-auto pr-6 custom-scrollbar scrollbar-gutter-stable">
        {history.length === 0 ? (
          <div className="h-full flex items-center justify-center text-sm text-slate-500 font-bold uppercase tracking-widest">
               ...
               return (
                 <li key={entry.id} className={`flex items-start gap-4 p-2 rounded-2xl transition-all ${entry.id === lastNewId ? 'animate-flash-glow' : ''}`}>
                   <div className={`mt-0.5 w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${
                     isInput ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 font-black' : 
                     isOutput ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 font-black' : 
                     'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-black'
                   }`}>
                     {isInput ? <ArrowUpRight className="w-3.5 h-3.5" /> : isOutput ? <ArrowDownRight className="w-3.5 h-3.5" /> : <Edit2 className="w-3 h-3" />}
                   </div>
                   <div className="min-w-0 flex-1">
                     ...
                     <p className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mt-0.5">
                       {new Date(entry.created_at).toLocaleDateString()} {new Date(entry.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                     </p>
                   </div>
                   <div className="text-right text-sm font-black flex-shrink-0 tabular-nums">
                     <span className={isInput ? 'text-emerald-500' : isOutput ? 'text-blue-600' : 'text-slate-500 dark:text-slate-400'}>
                       {diff > 0 ? '+' : ''}{diff}
                     </span>
                   </div>
                 </li>
